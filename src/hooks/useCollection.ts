...
const LOCAL_KEY = "perfumisto_collection";
...
export function useCollection() {
  ...
  const add = useCallback(async (fragranceId: string) => {
    ...
    await supabase.from("collections").upsert({ user_id: user.id, fragrance_id: fragranceId });
  }, [user]);
  ...
  const remove = useCallback(async (fragranceId: string) => {
    ...
    await supabase.from("collections").delete().eq("user_id", user.id).eq("fragrance_id", fragranceId);
  }, [user]);

  const isInCollection = useCallback((fragranceId: string) => items.includes(fragranceId), [items]);

  return { items, loading, add, remove, isInCollection };
}
